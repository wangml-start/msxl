...
public class SKLineContent extends RelativeLayout{
    private SKlineChart chart;
    protected KlineGroup data;
    private ScaleGestureDetector mScaleGestureDetector = null;
    private GestureDetector mGestureDetector;

    //记录滑动值
    private static final String TAG = SKLineContent.class.getSimpleName();
    public SKLineContent(Context context) {
        this(context, null);
    }

    public SKLineContent(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SKLineContent(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        chart = new SKlineChart(context);
